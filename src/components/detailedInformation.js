...
import {Button, WhiteSpace, WingBlank, Modal, PullToRefresh, Popover, Accordion, InputItem, Toast} from 'antd-mobile';
...
class DetailedInformation extends React.Component {
        ...
        axios({
            ...
            if (response.status === 200 && response.data.status === 1) {
                ...
                if (res) {
                    this.setState({
                        status: res.status,
                        coverPic: (res.coverPic && (config.publicStaticUrl + res.coverPic)) || ('../images/cover.png'),
    ...
    componentWillMount() {
        ...
        //client:1用户端 2医生端
        const {queryContentType, isShare, client} = this.props.match.params;
        this.setState({
    ...
    */
    ...
    //各种人数+1
    peopleNumberAddOne(educationId, type, drOrUser = 1) {
        //drOrUser 1 用户端 2 医生端
        //educationId 内容Id
        // type类型 3：问答(语音) 5:文章 6 视频 7直播
        axios({
            url: URLconfig.publicUrl + 'appEducation/educationView.do',
            method: 'post',
            data: {
                "drOrUser": drOrUser,
                "type": type,
    ...
    getAudiosState(isPlaying, audiosIsClick, id) {
        if (isPlaying) {
            this.peopleNumberAddOne(this.state.audioId, 3);
        }
        this.setState({
            isPlaying: isPlaying,
    ...
    //点击中间医生介绍部分到医生详情
    toDocDetail = (isShare, docId,doctorId,type) => {
        if (isShare !== 1) {
            if(parseInt(type)===2){
                this.props.history.push({pathname: `/doctorDetailedInformation/${doctorId}/0/${this.props.match.params.client}`,});
            }
            else{
                this.props.history.push({pathname: `/doctorDetailedInformation/${docId}/0/${this.props.match.params.client}`,});
            }
        }
        else{
            if(parseInt(type)===2){
                this.props.history.push({pathname: `/doctorDetailedInformation/${doctorId}/1/${this.props.match.params.client}`,});
            }
            else{
                this.props.history.push({pathname: `/doctorDetailedInformation/${docId}/1/${this.props.match.params.client}`,});
            }
        }
    };
    ...
    changeFocus = (e) => {
        e.stopPropagation();
        const {contentType, attentionIds, doctorId, userId} = this.state;

        if (attentionIds <= 0) {
            //未关注
            if (doctorId) {
                this.payFocus(doctorId);
            }
            if (userId) {
   ...
   */
            ...
            return (<div
                ...
                { // 对map 循环出来的每个属性插入标签元素
                    innerContent.map((inner, index) => {
                        return (
                            <div style={{whiteSpace: 'pre-wrap'}} className={'showDetailResume'} key={index}>
                                {inner.content}
                                {inner.title}
    ...
    _setInnerContent(synopsis, contentList, content, isWxDesc) {
        ...
        if (isWxDesc) {
            return (inner[1] && inner[1].content) || (inner[0] && inner[0].content) || (inner[2] && inner[2].content)
        }
    }
    ...
    showAlertWhenDidNotPay(isgratis, amout, ispay, image, audios, id) {
        // isgratis 1:免费 2:不免费
        // ispay是否支付 1未支付 2 已支付
        if (ispay === 1 && isgratis === 2) {
            ...
            if (this.state.isShare === 1) {
                return <img className={'setImage'} src={config.publicStaticUrl + image} alt="" onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = require("../images/cover.png")
                }}/>
            } else {
                return <div className={"videoPlayer"} style={{position: 'relative'}} onClick={() => this.play(id)}>
                    <Player
                        playsInline
    ...
    diffContentTypeArea(collectedIds, type, contentType, status, liveStatus, startingTime, isgratis, fee, ispay, collectNum, dzNum, isDz, isEnroll) {
        ...
        if (this.state.isShare) {
            return <DownloadArea whereIs={'detailModule'} contentType={contentType}
   ...
   /*     setTimeout(()=>{
    ...
    //跳转到评论列表
    toCommentList = (postType, commentId, isExtend) => {
        const {collectNum, dzNum, isDz, collectedIds} = this.state;
        const {id} = this.state;

        this.props.history.push({pathname: `/commentList/${id}/${postType}/${commentId}/${collectNum}/${dzNum}/${isDz}/${collectedIds}/${isExtend}/0`,});
        ...
        if (commentsList.length) {
                    ...
                    <div className={'firstClass'}>
                        <div className={'commentsAll'}>
                            ...
                            <div className={'commentsInner'}
                                 onClick={() => this.replyClass(item.userName, item.glandularArticleId, item.commentatorType, item.postType, item.userId, item.replyUserType, item.id)}>{item.content}
                                {/*<span className={'commentsInnerResponseNum'} onClick={() => {
                                    this.toCommentList(item.postType, item.id, isExtend)
                                }}>{item.childrenNum} 回复</span>*/}
                                <span className={this.state.isShare === 1 ? 'hide' : 'commentsInnerResponseNum'}
                                      onClick={() => {
                                          this.toCommentList(item.postType, item.id, isExtend)
                                      }}>{item.childrenNum} 回复</span>
                            </div>
                            ...
                            item.childrenReplys.map((item1, key1) =>
                                <div key={key1} className={'secondClass'}>
                                    <div className={'commentsAll'}>
                                        <div className={'commentsHeader'}>
                                            <div className={'commentsName'}>
                                                <img src={item1.headicon&&item.headicon.indexOf('http') !== -1 ?item1.headicon : config.publicStaticUrl+item1.headicon} alt="" onError={(e) => {
                                                    e.target.onerror = null;
                                                    e.target.src = require("../images/cover.png")
                                                }}/>
            ...
            if (this.state.contentType !== 7) {
                return <div className={'onePieceOfComment'}>
                    <div className={'firstClass'}>
                        <div className={'commentsAll'} style={{textAlign: 'center', color: 'rgba(165,165,165,1)',}}>
                            暂时没有评论，来做第一只吃螃蟹的人吧！
                        </div>
                    </div>
                </div>
    ...
    render() {
        ...
        const _showPeopleNumber = (...args) => {
            ...
            let contentType = args[0], status = args[1], enrollNum = args[2], onlineNum = args[3], viewCount = args[4],
                audiosIsClick = args[5], startingTime = args[6],readingCount=args[7];
            ...
            if (contentType === 7 && status === 40) {
                return (<div className={'reportNumber'}>{readingCount}人观看</div>)
            }
            if (contentType === 5) {
        ...
        const _isEliteOrRecommendation = (elite, recommendation, audiosIsClick) => {
            ...
            if (elite === 1 && recommendation === 0 && !audiosIsClick) {
                return <div style={{right: '3%'}} className={'eliteTags'}>精华</div>
            }
        ...
        const showImageCover = (contentType, isPlaying, id) => {
            // 3：问答   5:文章  6 视频 7 直播
            ...
            if (contentType === 7) {
                ...
                return <img src={coverPic.replace(" ", "")} alt="" className={'coverPic'} onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = require("../images/cover.png")
                }}/>
            }
            ...
            if (contentType === 6) {
                return this.showAlertWhenDidNotPay(isgratis, amout, ispay, image, audios, id)
            }
        };
        ...
        return (
            ...
            <div className={'setDetailedInformation'}>
                ...
                {isShare === 1 ? <DownloadTips client={this.props.match.params.client} whereIs={'detailModule'}
                ...
                >
                    <div>
                            ...
                            >
                                <div className={'modalContent'}>
                                    ...
                                    {this.state.isShare === 1 ?
                                        <div className={'modelPriceShare'}>仅需￥{amout}，<p>请进入公众号或者APP后购买</p></div> :
                                        <div className={'modalPrice'}>仅需￥{amout}，点击下方立即购买</div>}
                                    {this.state.isShare === 1 ? '' : <Button className={"buyNow"} onClick={() => {
                                        this.BuyItNow(this.state.id, contentType, isEnroll, isgratis)
                                    }}>立即购买</Button>}
                                    {/*
                                    <div className={'modalPrice'}>仅需￥{amout}，点击下方立即购买</div>
                        ...
                        <div className={'coverImageArea'}>
                            ...
                            {_showPeopleNumber(contentType, status, enrollNum, onlineNum, viewCount, audiosIsClick, startingTime,readingCount)}
                            ...
                            {showImageCover(contentType, isPlaying, id)}
                        </div>
                        ...
                        <div className={'secondPart'} onClick={() => {
                            this.toDocDetail(isShare, userId,doctorId,this.props.match.params.queryContentType)
                        }}>
                            ...
                            <div className={'focusArea'}>
                                {isShare === 1 ? '' : <div onClick={(e) => this.changeFocus(e)}>
                        ...
                        <div
                            className={`${this.state.contentType !== 7 ? "forthPart fixedForthPartBottom" : ""} ${this.state.isShare === 1 ? 'shareForthPart' : ''}`}>
                            {contentType !== 7 ? <div className={'comments'}>评论({replyNum})</div> : ''}
                        </div>

                        <div
                            className={`${this.state.beComments === 'yes' ? "forthPart fixedForthPartTop" : ""} ${this.state.isShare === 1 ? 'shareForthPart' : ''}`}>
                            {this.state.beComments && this.state.beComments === 'yes' ? this.setComments(commentsList) : ''}

                            {commentsList.length ? <div className={'addFKUBottom'}>-已经到底部啦-</div> : ''}
                        </div>
                ...
                <div className={isShare ? '' : 'buttonArea'}>
                    {this.diffContentTypeArea(collectedIds, type, contentType, status, liveStatus, startingTime, isgratis, fee, ispay, collectNum, dzNum, isDz, isEnroll)}
                </div>
