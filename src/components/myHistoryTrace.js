...
class myHistoryTrace extends Component {
    constructor(props) {
        super(props);
    ...
    genData = (isSearch, type = 0) => {  //请求数据的方法
        ...
        return fetch(config.publicUrl + `app/historicalRecord.do?page=${this.pageNo}&limit=5&queryContentType=${type}&title=${this.state.title}`,
            {
                method: 'POST',
                headers: {
    ...
    //等待删除
    waitingAxiosUpdate = (deleteStatus, type = 'notExist', message = 'loading...') => {
        if (deleteStatus === true && type === 'exist') {
            Toast.loading(message, 2);
        }
        if (deleteStatus === false && type === 'noExist') {
            Toast.success(message, 2);
        }
    };

    //删除历史足迹
    deleteHistoryTrace(id, queryContentType) {
        alert('历史足迹', '您要取消该历史吗？', [
            {
                text: '是的',
                onPress: () => {
                    this.setState({
                        ...
                        if (id) {
                            //取消收藏
                            this.waitingAxiosUpdate(this.state.deleteStatus,'exist','删除中，请稍等...')
                            axios({
                                url: URLconfig.publicUrl + '/app/deleteHistoricalRecord.do',
                                method: 'post',
                                data: {
                                    "educationId": id,
                                ...
                                if (response.status === 200 && response.data.status === 1) {
                                    if (response.data.status === 1) {
                                        this.setState({
                                            ...
                                            this.waitingAxiosUpdate(this.state.deleteStatus,'noExist','删除成功')
...
// 其余状态：0-待审核；1-审核通过；2-审核关闭；3-审核不通过; 4:已删除',
                ...
                <SwipeAction
                    ...
                    right={[
                            ...
                            },*/
                        {
                            text: '删 除',
                            onPress: () => {
                                this.deleteHistoryTrace(rowData.id, rowData.queryContentType)
        ...
        return (
            <ErrorBoundary>
                ...
                <div className='livelisttitlebox entryRecord myCollection myHistoryTrace' style={{background: 'rgba(238,242,249,1)'}}>
                    <HeaderNavBar title={'我的历史足迹'} isLight={'navBarHeaderLight'} noIcon={true}/>
